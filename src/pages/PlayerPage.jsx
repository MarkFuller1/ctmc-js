...
const PlayerPage = observer(
  class PlayerPage extends React.Component {
      ...
      if (this.state.badges === []) {
        ...
        if (this.state.badges.length) {
          ...
          for (let i = 0; i < this.state.badges.length; i++) {
            bdArr.push(<Chip label={this.state.badges[i]["awardID"]} />);
          }
        }
        return bdArr;
      }
      ...
      return (
        <div>
          ...
          >
            ...
            <Grid item>{this.renderBadges()}</Grid>
            <div style={{ width: "80vw" }}>
              ...
              <TabPanel value={this.state.value} index={0}>
                Place stats concerning overall wins, teams, parks, play time,
                salary
                <GeneralPlayerStats
    ...
    async componentDidMount() {
      ...
      const badgeResponse = await Request.getPlayerBadges(
        this.props.match.params.playerid
      );

      if (imageURLResponse[0] !== undefined) {
